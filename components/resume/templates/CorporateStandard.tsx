...
export const CorporateStandard: React.FC<CorporateStandardProps> = ({ data }) => {
	const {
		personal = {
			fullName: '',
			jobTitle: '',
	...
	if (personal.email) {
		contactLinks.push(
			<a key="email" href={`mailto:${personal.email}`} className="text-[#1e293b] hover:underline">
				{personal.email}
			</a>,
		);
	}
							...
							<div key={exp.id || idx} className="break-inside-avoid">
								<div className="flex items-baseline justify-between text-[10pt]">
									<span className="font-bold text-slate-900 font-sans">
										{exp.role} <span className="font-normal text-slate-600">—</span> {exp.company}
										{exp.location && (
						...
						{projects.map((proj, idx) => (
							<div key={proj.id || idx} className="break-inside-avoid">
								<div className="text-[10pt] font-sans">
									...
									{proj.technologies && (
										<span className="text-slate-600 font-normal"> — {proj.technologies}</span>
									)}
								</div>
